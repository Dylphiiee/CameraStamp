...
object StampRenderer {
    ...
    private fun textPaint(size: Float, scale: Float): Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.WHITE
        textSize = size
        typeface = Typeface.create(Typeface.DEFAULT, Typeface.BOLD)
        style = Paint.Style.FILL
        // A thin 1px drop shadow keeps text readable on busy/light backgrounds without
        // the thick black outline (or blurry glow) look that was flagged as distracting.
        setShadowLayer(1f * scale, 1f * scale, 1f * scale, Color.argb(190, 0, 0, 0))
    }
    ...
    private fun drawStamp(canvas: Canvas, widthPx: Int, heightPx: Int, data: StampData) {
        ...
        val locationFill = textPaint(26f * scale, scale)
        val dayFill = textPaint(34f * scale, scale)
        val timeFill = textPaint(58f * scale, scale)
        val companyFill = textPaint(20f * scale, scale)
        ...
        for (i in locationLines.indices.reversed()) {
            canvas.drawText(locationLines[i], marginX + 24f * scale, lineY, locationFill)
        ...
        canvas.drawText(data.dayText, dateDayX, dayBaselineY, dayFill)
        canvas.drawText(data.dateText, dateDayX, dateBaselineY, dayFill)

        // Big time text shares the day line's baseline (to its left, before the divider bar).
        canvas.drawText(data.timeText, marginX, dayBaselineY, timeFill)
        ...
        canvas.drawText(data.companyName, marginX, companyBaselineY, companyFill)
